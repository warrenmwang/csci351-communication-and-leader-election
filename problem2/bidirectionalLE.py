    ...
    while True:
        ...
        if probe:
            ...
            if recvID > rank and distance < pow(2, rndNum):
                ...
                comm.isend((recvID, rndNum, distance+1, True, False, terminateNonLeader), dest = neighbors[0])
                comm.isend((recvID, rndNum, distance+1, True, False, terminateNonLeader), dest = neighbors[1])
                numMessages+=2

            if recvID > rank and distance >= pow(2, rndNum):
                #send reply, j, r, to left/right (resp.)
                comm.isend((recvID, rndNum, distance, False, True, terminateNonLeader), dest = neighbors[0])
                comm.isend((recvID, rndNum, distance, False, True, terminateNonLeader), dest = neighbors[1])
                numMessages+=2
        ...
        if reply:
            ...
            #already received reply, j, r from right/left (resp.)
                ...
                comm.isend((rank, rndNum+1, 1, True, False, terminateNonLeader), dest = neighbors[1])
                numMessages+=2
